...
#### 高并发解决方案
1. **Redis库存预热** — 活动库存提前加载到 Redis，降低 DB 压力
2. **Lua脚本原子扣减** — Redis 层原子扣库存，防超卖
3. **固定窗口限流** — 按活动维度限制每秒请求数（非严格令牌桶）
4. **用户购买标记防重** — Redis `SET NX` 防止同一用户重复抢
5. **RabbitMQ异步削峰** — 下单请求异步落库
6. **Redisson + DB乐观锁** — 消费者侧按订单号加锁、扣减 DB 秒杀库存；失败回补 Redis
7. **消息失败回补** — MQ 发送失败或落库失败时回补 Redis 库存并清除用户标记

> 说明：项目引入了 Spring Cloud Alibaba BOM，但当前实现仍使用 **Eureka**，未接入 Nacos/Sentinel/Seata。
